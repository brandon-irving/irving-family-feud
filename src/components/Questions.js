...
export const getQuestionsHook = () => {
    const [mutatedQuestions, setmutatedQuestions] = useState([])
    const [loading, setloading] = useState(true)
    const [error, seterror] = useState(null)

    async function getQuestions() {
        try {
            const questionsArrayResponse = await fetch('https://s3-us-west-2.amazonaws.com/s.cdpn.io/40041/FF3.json')
            const questions = await questionsArrayResponse.json()
            const mutatedQuestions = mutateQuestions(questions)
            setmutatedQuestions(mutatedQuestions)
        } catch (error) {
            seterror(error)
        }
        setloading(false)
    ...
    return { loading, questions: [...mutatedQuestions], error }
}

function mutateQuestions(questionsArray) {
    const questions = Object.keys(questionsArray)
    const mutatedQuestions = questions.map((question, i) => {
        const choices = questionsArray[question].map(choice => {
